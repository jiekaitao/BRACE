...
### Challenge: Crash Analysis Upload Pipeline Integration
Wiring the "Upload for Crash Analysis" button on the landing page required building a complete offline video processing pipeline separate from the existing real-time WebSocket flow and the basketball game analysis pipeline. Key challenges:

1. **Module promotion**: Paul's collision detection and biomechanics model code lived in `scripts/` as standalone scripts. They had to be copied to `backend/` and made importable with Docker bind-mounts. The `try/except` import pattern (backend.foo vs foo) was essential for running both in Docker (flat `/app/` mount) and in tests (normal Python path).

2. **Testing without GPU**: The `main.py` module triggers the full FastAPI app startup (YOLO, TensorRT, MongoDB directory creation) on import. Tests for `ClosingSpeedTracker` (defined in main.py) couldn't import it directly outside Docker. We solved this by extracting the class source via regex and `exec()` at test collection time, avoiding the heavyweight import chain entirely.

3. **Collision detection cooldown**: Without a cooldown mechanism, the collision detector fires on every frame while two overlapping skeletons remain close, producing hundreds of duplicate events for a single collision. We added a per-pair cooldown of 15 frames (~0.5s) to deduplicate events.

4. **Synthetic skeleton test fixtures**: Building realistic two-person collision scenarios for tests required generating COCO-17 keypoint arrays with correct spatial relationships (bboxes overlapping, head positions converging, confidence values above the 0.3 visibility threshold). The existing `test_movement_quality.py` pattern of sinusoidal motion didn't apply — crash tests needed two subjects with controlled approach trajectories.
