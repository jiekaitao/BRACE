...
.venv/bin/python -m pytest tests/ -v                    # Run all 292 tests
.venv/bin/python -m pytest tests/test_srp.py -v         # Single test file
.venv/bin/python -m pytest tests/test_anomaly.py::test_normal_scores_low  # Single test
```
...
StreamingAnalyzer.process_frame(landmarks):
  ...
  ↓
MultiSubjectFrameResponse JSON → WebSocket → frontend
  ↓
useAnalysisWebSocket hook → subjectsRef (Map<trackId, SubjectState>)
  ↓
Canvas/Three.js reads subjectsRef at 60fps (no React re-renders)
```
...
Mixed pattern to avoid re-rendering at 60fps:
...
- `backend/main.py` — FastAPI server, WebSocket frame loop, video loop detection
...
- `backend/gemini_classifier.py` — Gemini 2.0 Flash activity labels (google-genai SDK, NOT google-generativeai)
...
- `frontend/src/components/SkeletonGraph.tsx` — Hip-centered skeleton with depth-based rendering
...
- `frontend/src/lib/types.ts` — All TypeScript interfaces (SubjectState, PipelineResult, FrameQuality, etc.)

## Important Patterns

- **Docker import paths**: Backend files are bind-mounted flat to `/app/` in Docker, not `/app/backend/`. Use try/except for cross-module imports: `try: from backend.foo import Bar` / `except ImportError: from foo import Bar`. Adding a new bind-mount requires `docker compose up -d backend` (recreate), not just `restart`.
...
| `GOOGLE_GEMINI_API_KEY` | — | Gemini 2.0 Flash for activity classification |
...
336 tests using synthetic skeleton data (sinusoidal motion with fixed anchor joints). venv at `.venv/`. All tests must pass before any commit. Key test files: `test_movement_quality.py` (99), `test_movement_guidelines.py` (33), `test_motion_clustering.py` (23), `test_gemini_classifier.py` (22), `test_botsort_tracker.py` (16), `test_identity_resolver.py` (15), `test_pipeline_interface.py` (20).
